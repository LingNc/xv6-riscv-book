...
当使用页表中没有映射的虚拟地址，或者使用映射的 \lstinline{PTE_V} 标志被清除的地址，或者映射的权限位（\lstinline{PTE_R}、\lstinline{PTE_W}、\lstinline{PTE_X}、\lstinline{PTE_U}）禁止正在尝试的操作时，RISC-V CPU 会引发页错误异常。RISC-V 区分三种页错误：加载页错误（由加载指令引起）、存储页错误（由存储指令引起）和指令页错误（由取指执行指令引起）。\lstinline{scause} 寄存器指示页错误的类型，\indexcode{stval} 寄存器包含无法翻译的地址。

页表和页错误的组合是一个强大的工具。页表给内核提供了虚拟地址和物理地址之间的一层间接性，使内核能够控制地址空间的结构和内容。页错误允许内核拦截加载和存储操作，并通过修改页表，实时指定这些引用指向的数据。内核可以利用这些能力提高效率：例如，写时复制 fork 允许内核在父进程和子进程之间透明地共享内存，避免了复制任何一方都不写入的页面的开销。应用程序程序员也可以受益。一种可能性是内存映射文件，内核使用分页使文件内容出现在应用程序的地址空间中，以响应页错误透明地读取文件块。另一种是惰性内存分配，允许程序请求巨大的虚拟地址空间，但只为程序实际读写页面支付分配物理内存的成本。xv6 仅将页错误用于一个目的：惰性分配。
...
xv6 的 \indextext{lazy allocation} 有两部分。首先，当应用程序通过调用带有 \lstinline{SBRK_LAZY} 标志的 \lstinline{sbrk} 请求内存时，内核记录大小的增加，但不分配物理内存，也不为新范围的虚拟地址创建 PTE。其次，在这些新地址之一上发生页错误时，内核分配一页物理内存并将其映射到页表中。内核透明地实现惰性分配：应用程序无需修改即可受益。

惰性分配对应用程序很方便，因为它们不必准确预测需要多少内存。例如，应用程序可能处理输入，但事先不知道输入有多大。使用惰性分配，应用程序可以为最坏情况请求内存，但不必为此最坏情况付费：内核不必为应用程序从未使用的页面做任何工作。

此外，如果应用程序要求大幅增加地址空间，那么没有惰性分配的 \lstinline{sbrk} 会很昂贵：如果应用程序请求一 GB 内存，内核必须分配并清零 262,144 个 4096 字节的物理页面。惰性分配允许将这种成本分散到时间上。另一方面，惰性分配会产生页错误的额外开销，这涉及用户/内核转换。操作系统可以通过每次页错误分配一批连续页面而不是一个页面，以及为这种页错误专门化内核进入/退出代码来降低这种成本（尽管 xv6 两者都不做）。

另一方面，当为惰性分配页面发生页错误时，内核可能发现它没有空闲内存可分配。在这种情况下，内核没有简单的方法向应用程序返回内存不足错误，而是杀死应用程序。对于希望在分配失败时收到错误的应用程序，xv6 允许应用程序通过使用 \lstinline{SBRK_EAGER} 标志调用 \lstinline{sbrk} 来立即分配内存。
...
当进程加载或存储到缺少有效页表映射的虚拟地址时，CPU 将引发 \indextext{page-fault exception}。
\lstinline{usertrap} 检查这种情况
\lineref{kernel/trap.c:/page fault/}
并调用 \lstinline{vmfault}
\lineref{kernel/vm.c:/^vmfault/}
来处理页错误。\lstinline{vmfault}
检查错误地址是否在 {\tt sbrk} 先前授予的区域内，
...
请注意，xv6 使用进程的页表不仅告诉硬件如何映射用户虚拟地址，还作为分配给该进程的物理内存页面的唯一记录。这就是在 {\tt uvmunmap} 中释放用户内存需要检查用户页表的原因。
...
写时复制 fork 通过适当使用页表权限和页错误使父进程和子进程安全地共享物理内存。基本计划是让父进程和子进程最初共享所有物理页面，但每个都将其映射为只读（\lstinline{PTE_W} 标志清除）。然后父进程和子进程都可以从共享物理内存中读取。如果任何一方写入共享页面，RISC-V CPU 会引发页错误异常。支持 COW 的内核会响应分配新物理内存页面并将共享页面复制到新页面中。然后内核会更改错误进程的页表中的相关 PTE 以指向副本并允许读写，然后在导致错误的指令处恢复错误进程。因为 PTE 现在允许写入，重新执行的存储指令将不会出错，并将修改页面的私有副本而不是共享页面。

写时复制需要记账来帮助决定何时可以释放物理页面，因为每个页面可以根据 fork、页错误、exec 和 exit 的历史被不同数量的页表引用。这种记账允许一个重要的优化：如果进程产生存储页错误且物理页面仅从该进程的页表引用，则不需要复制。
