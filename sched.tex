...
任何操作系统运行的进程数都可能超过计算机的 CPU 数量，
因此需要一种机制来在进程之间
\indextext{多路复用（multiplexing）}
CPU。理想情况下，这种共享对用户进程应该是透明的。
一种常见的方法是通过将进程
多路复用硬件 CPU，为每个进程营造它拥有自己虚拟 CPU 的假象。
本章解释 xv6 如何实现这种多路复用。
...
Xv6 通过两种情况的 CPU 切换来实现多路复用：。
首先，当进程发出阻塞式系统调用（需要等待）时，
...
基本思想是保存和恢复 CPU 寄存器，
虽然这无法用 C 语言实现，因此变得棘手。
其次，如何以对用户进程透明的方式强制切换？Xv6 使用标准技术，
即使用硬件定时器触发中断来驱动上下文切换。
第三，所有 CPU 都在同一组进程之间切换，因此需要一种锁机制来避免错误，
