...
任何操作系统运行的进程数都可能超过计算机拥有的 CPU 数量，
因此需要一个计划来在进程之间
\indextext{多路复用（multiplexing）}
CPU。理想情况下，这种共享对用户进程应该是透明的。
一种常见的方法是通过将进程
多路复用到硬件 CPU 上，为每个进程提供它拥有自己的虚拟 CPU 的幻觉。
本章解释 xv6 如何实现这种多路复用。
...
Xv6 在两种情况下将每个 CPU 从一个进程切换到另一个进程来实现多路复用。
首先，当进程进行阻塞的系统调用（必须等待）时，
例如 \lstinline{read} 或 \lstinline{wait}，xv6 会进行切换。
其次，xv6 定期强制切换以应对长时间计算而不阻塞的进程。
前者称为自愿切换，
后者称为非自愿切换。

实现多路复用提出了一些挑战。首先，如何从一个进程切换到另一个进程？
基本思想是保存和恢复 CPU 寄存器，
尽管这无法用 C 语言表达，使其变得棘手。
其次，如何以对用户进程透明的方式强制切换？Xv6 使用标准技术，
即用硬件定时器的中断驱动上下文切换。
第三，所有 CPU 都在同一组进程之间切换，因此需要一个锁定计划来避免错误，
例如两个 CPU 决定同时运行同一个进程。
第四，当进程退出时，必须释放其内存和其他资源，
但它自己无法完成所有这些工作。
第五，多核机器的每个 CPU 必须记住它正在执行哪个进程，
以便系统调用影响正确的进程的内核状态。
...
术语``上下文切换（context switch）''指的是 CPU 停止执行一个内核线程（通常稍后恢复）并恢复执行不同内核线程所涉及的步骤；
这种切换是多路复用的核心。Xv6 不直接从一个进程的内核线程上下文切换到另一个进程的内核线程；
相反，内核线程通过上下文切换到该 CPU 的``调度器线程（scheduler thread）''来放弃 CPU，
然后调度器线程选择一个不同的进程的内核线程来运行，并上下文切换到该线程。
...
在更广泛的范围内，从一个用户进程切换到另一个用户进程所涉及的步骤如图~\ref{fig:switch} 所示：
从旧进程的用户空间陷入（系统调用或中断）到其内核线程，
上下文切换到当前 CPU 的调度器线程，
上下文切换到新进程的内核线程，
然后陷入返回到用户级进程。
...
{\tt kernel/swtch.S} 中的函数 {\tt swtch()} 包含线程上下文切换的核心：
它保存被切换线程的 CPU 寄存器，并恢复被切换线程之前保存的寄存器。
这足够的基本原因是线程的状态由内存中的数据（例如其栈）加上其 CPU 寄存器组成；
线程内存不需要保存和恢复，因为不同线程将数据保存在 RAM 的不同区域；
但 CPU 只有一组寄存器，因此必须在线程之间切换（保存和恢复）它们。

% why must it be in assembler? why not C?
% why swtch(), not switch()?

每个线程的 {\tt struct proc} 包含一个 {\tt struct context}，
用于保存线程不运行时的已保存寄存器。CPU 调度器线程的 {\tt struct context} 位于该 CPU 的 {\tt struct cpu} 中。
当线程 X 希望切换到线程 Y 时，线程 X 调用 {\tt swtch(\&X's context, \&Y's context)}。{\tt swtch()}
将当前 CPU 寄存器保存在 X 的上下文中，然后将 Y 的上下文内容加载到 CPU 寄存器中，然后返回。
...
{\tt a0} 保存第一个函数参数，{\tt a1} 保存第二个参数；
在这种情况下，是两个 {\tt struct context} 指针。
{\tt 16(a0)} 指的是从 {\tt a0} 指向的内存偏移 16 字节；
参考 {\tt kernel/proc.h} 中 {\tt struct context} 的定义
\lineref{kernel/proc.h:/^struct.context/}，
这是称为 {\tt s0} 的结构体字段。

{\tt swtch} 的 {\tt ret} 返回到哪里？它返回到 {\tt ra} 寄存器指向的指令。
在线程 X 调用 {\tt swtch()} 切换到 Y 的示例中，当 {\tt ret}
执行时，{\tt ra} 刚从 Y 的 {\tt struct context} 加载。
而 Y 的 {\tt struct context} 中的 {\tt ra} 最初是由 Y 调用 {\tt swtch} 时保存的，
当时 Y 放弃了 CPU。因此 {\tt ret} 返回到 Y 调用 {\tt swtch()} 之后的指令；
也就是说，X 调用 {\tt swtch()} 返回，就好像从 Y 最初调用 {\tt swtch()} 返回一样。
而且 {\tt sp} 将是 Y 的栈指针，因为 {\tt swtch} 从 Y 的 {\tt struct context} 加载了 {\tt sp}；
因此在返回时，Y 将在自己的栈上执行。
{\tt swtch()} 不需要直接保存或恢复程序计数器；
保存和恢复 {\tt ra} 就足够了。
...
Xv6 经常需要指向当前进程的 \lstinline{proc} 结构体的指针。
在单处理器上，可以有一个全局变量指向当前 \lstinline{proc}。
这在多核机器上不起作用，因为每个 CPU 执行不同的进程。
...
xv6 调度器实现了一个简单的调度策略，轮流运行每个进程。
这种策略称为
\indextext{轮询（round robin）}。
真实操作系统实现更复杂的策略，例如，允许进程具有优先级。
