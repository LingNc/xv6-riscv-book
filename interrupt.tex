...
是驱动程序结构的简单示例。
控制台驱动程序通过连接到 RISC-V 的 \indextext{UART}
串口硬件接收人类输入的字符。
控制台驱动程序一次累积一行输入，
处理特殊输入字符如
退格键和控制键-u。用户进程，如 shell，使用
...
每个输入字节时生成
接收中断，
并在 UART 完成发送每个输出字节时生成
\indextext{传输完成（transmit complete）}
...
键入的字符）时尤为重要。这个想法有时被称为 \indextext{I/O
  并发（I/O concurrency）}。
...
保证在有限时间内发生的响应。
例如，在安全关键系统中，错过截止日期可能导致
灾难。Xv6 不适合实时环境。除了其他问题，
xv6 的调度器在决定接下来运行什么进程时没有考虑实时截止日期，
而且 xv6 有很长的禁用中断的内核代码路径，因此它可能不会快速响应中断。
实时操作系统不仅要修复这些问题，还要以一种允许分析最坏情况响应时间的方式构建。
