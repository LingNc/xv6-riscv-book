...
而且读取磁盘的进程需要等待
磁盘硬件完成读取。
Xv6 内核在这些情况（以及许多其他情况）下使用一种称为睡眠和唤醒的机制。
睡眠允许内核线程等待某个条件变为真；
另一个线程或中断处理程序可以使条件变为真（通常通过修改某些变量），
然后调用唤醒来指示等待该条件的线程应该恢复执行。
睡眠和唤醒通常被称为
...
\lstinline{sleep()} 将调用进程标记为 \lstinline{SLEEPING}
（而不是 \lstinline{RUNNABLE}），并通过上下文切换到调度器来释放 CPU，
以便其他进程可以运行。
...
即将睡眠的进程在检查条件之前直到标记自己为 \texttt{SLEEPING} 之后持有
条件锁或它自己的
\lstinline{p->lock}
或两者；
参见图~\ref{fig:overlap}。
...
唤醒父进程以防它在 \lstinline{wait} 中，将调用者标记为僵尸，并永久让出 CPU。
...
和
\lstinline{signal}。
所有这些机制都有相同的特点：睡眠条件由某种在睡眠期间原子丢弃的锁保护。
